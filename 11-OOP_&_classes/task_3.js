/*
Теперь, вместо того чтобы указывать профессию в объекте класса `Employee`, создайте подклассы для разных типов сотрудников —
`Developer`, `Manager`, и `Designer`, — которые будут наследовать класс `Employee`.
...
2. Создайте подклассы `Developer`, `Manager`, и `Designer`:
Каждый из этих подклассов будет наследовать от класса `Employee` и добавлять свои специфические поля.
