...
`addEmployee(employee)` — добавляет сотрудника в массив `employees` с проверкой, что переданный объект является экземпляром класса `Employee`. (Подсказка: `instanceOf`)
